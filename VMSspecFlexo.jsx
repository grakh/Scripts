...
BotOfFiles = File (SCRIPTS_FOLDER + '/VMSFlexo.ai');
...
info: Group { orientation: 'row', alignChildren: 'top',\
	panel1: Panel {text: 'Заказ:', preferredSize: [370,170], alignChildren: 'right',\
		list00: Group { orientation: 'row', \
			...
			listbox: EditText {text:'Ф-', preferredSize: [300,20]}, \
		} \
		...
		list03: Group { orientation: 'row', \
			...
			text: StaticText {text:',', preferredSize: [50,20]}, \
			text: StaticText {text:'Намотка: '}, \
			listbox2: EditText {text:'0', preferredSize: [45,20]}, \
		} \
...
switch (Ops){
	...
	case  'cut':
	case  'Knife':
	case  'knife':	line.filled = false; line.stroked = true; line.strokeWidth = 0.3*mm;
...
switch (T){
    ...
    case  '#08':  newLayer1[j].textFrames[i].paragraphs[0].paragraphAttributes.justification = Justification.LEFT;
					newLayer1[j].textFrames[i].contents = elemSize[0]+"x"+elemSize[1]+" мм";
					break;
	...
	case  '#05':  newLayer1[j].textFrames[i].paragraphs[0].paragraphAttributes.justification = Justification.LEFT;
	  ...
	  var Namotka = parseInt(dlg.info.panel1.list03.listbox2.text);
...
function selAll(O, mm){
   ...
   for(var i = 0; i < documents[0].pathItems.length; i++){
	  ...
	  if (docSelection.strokeColor.spot.name == documents[0].swatches.getByName(O).name) {
		SAll[j] = parseInt((docSelection.width/mm).toFixed(1));
		j++;
		SAll[j] = parseInt((docSelection.height/mm).toFixed(1));
		j++;
